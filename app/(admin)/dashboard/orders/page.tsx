...
// PENGAMBILAN ENV MURNI (TANPA HARDCODE KUNCI)
...
export default function AdminOrdersFullList() {
  ...
  if (isLoading) return <div className="w-full h-[60vh] flex flex-col items-center justify-center text-primary"><Loader2 size={40} className="animate-spin opacity-50 mb-4" /><p className="text-xs font-bold uppercase tracking-widest text-gray-400 animate-pulse">Mengambil Data Transaksi...</p></div>;
    ...
    <div className="max-w-7xl mx-auto space-y-8 relative z-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Kelola status pesanan (Sukses/Batal) dan harga akhir.</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-white/5">
              {leads.map((lead) => (
                <tr key={lead.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-5 font-black text-white">Rp {Number(lead.total_price || 0).toLocaleString('id-ID')}</td>
                  <td className="p-5">{getStatusBadge(lead.status)}</td>
                  <td className="p-5 text-right">
                    <button onClick={() => { setEditingOrder(lead); setEditForm({ price: lead.total_price || 0, status: lead.status || 'pending' }); }} className="inline-flex items-center gap-2 bg-primary/10 text-primary hover:bg-primary hover:text-black px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-widest transition-all">
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button type="button" onClick={() => setEditForm({...editForm, status: 'pending'})} className={`py-3 text-xs font-bold rounded-xl border transition-all ${editForm.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' : 'bg-white/5 text-gray-500 border-transparent hover:bg-white/10'}`}>PENDING</button>
                  <button type="button" onClick={() => setEditForm({...editForm, status: 'completed'})} className={`py-3 text-xs font-bold rounded-xl border transition-all ${editForm.status === 'completed' ? 'bg-green-500/20 text-green-400 border-green-500/50' : 'bg-white/5 text-gray-500 border-transparent hover:bg-white/10'}`}>SUKSES</button>
                  <button type="button" onClick={() => setEditForm({...editForm, status: 'cancelled'})} className={`py-3 text-xs font-bold rounded-xl border transition-all ${editForm.status === 'cancelled' ? 'bg-red-500/20 text-red-400 border-red-500/50' : 'bg-white/5 text-gray-500 border-transparent hover:bg-white/10'}`}>BATAL</button>
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Harga Deal Akhir (Rp)</label>
                <input type="number" value={editForm.price} onChange={(e) => setEditForm({...editForm, price: Number(e.target.value)})} className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-primary focus:outline-none text-white font-bold" />
              </div>
